...
static ERL_NIF_TERM nif_open(ErlNifEnv *env, int argc,
  ...
  if (serial_open(res, speed) < 0) {
    ...
    return enif_make_tuple2(env, atom_er, enif_make_int(env, -1));
  } else
...
static ERL_NIF_TERM nif_read(ErlNifEnv *env, int argc,
  ...
  if (size < 0)
    return enif_make_tuple2(env, atom_er, enif_make_int(env, size));
  ErlNifBinary bin;
  ...
  if (serial_read(res, bin.data, bin.size) != bin.size) {
    return enif_raise_exception(
        env, enif_make_string(env, "serial_read failed", ERL_NIF_LATIN1));
  }
...
static ERL_NIF_TERM nif_write(ErlNifEnv *env, int argc,
  ...
  if (serial_write(res, bin.data, bin.size) != bin.size) {
    return enif_raise_exception(
        env, enif_make_string(env, "serial_write failed", ERL_NIF_LATIN1));
  }
  return atom_ok;
}
...
static ERL_NIF_TERM nif_close(ErlNifEnv *env, int argc,
  ...
  if (serial_close(res) < 0) {
    return enif_raise_exception(
        env, enif_make_string(env, "serial_close failed", ERL_NIF_LATIN1));
  }
  return atom_ok;
}
